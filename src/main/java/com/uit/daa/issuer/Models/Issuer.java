...
public class Issuer {
    ...
    public static Issuer getFromID(JdbcTemplate j) throws SQLException, NoSuchAlgorithmException{
             String sql = "select * from " + C.TB_CONFIG;
        Statement st = j.getDataSource().getConnection().createStatement();
        ResultSet rs= st.executeQuery(sql);
        if(rs.next()){
            BNCurve curve = BNCurve.createBNCurveFromName(rs.getString(C.CL_BNCURVE));
            IssuerSecretKey sk = new IssuerSecretKey(curve, rs.getString(C.CL_ISK));
            IssuerPublicKey pk = new IssuerPublicKey(curve, rs.getString(C.CL_IPK));
